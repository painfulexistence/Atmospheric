...
constexpr uint32_t kMagic = 0x31435441;// "ATC1"
...
// Left-neighbor prediction (first column predicts from the row above), zigzag
// varint residuals. Smooth heightfields produce mostly 1-byte residuals.
...
TerrainTileCache::TerrainTileCache(std::string dir) : _dir(std::move(dir)) {
    if (_dir.empty()) return;
    std::error_code ec;
    std::filesystem::create_directories(_dir, ec);
    if (ec) _dir.clear();// unwritable target (e.g. read-only bundle) — disable
}
...
void TerrainTileCache::Store(int tileX, int tileZ, int lod, uint32_t paramsHash, int w, const std::vector<float>& grid)
    const {
    if (_dir.empty()) return;
    ...
    Header h{ kMagic, paramsHash, static_cast<uint16_t>(w), static_cast<uint16_t>(w),
