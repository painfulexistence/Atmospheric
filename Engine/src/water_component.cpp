...
WaterComponent::WaterComponent(GameObject* owner, const WaterProps& props) : _props(props) {
    ...
    _mesh = am.CreatePlaneMeshSubdivided("Water_" + owner->GetName(), props.width, props.depth, props.subdivisions);
...
void WaterComponent::OnTick(float /*dt*/) {
    if (auto* wm = dynamic_cast<WaterMaterial*>(_material)) wm->waterLine = gameObject->GetPosition().y;
}
...
void WaterComponent::OnAttach() {
    float line = (_props.waterLine > -1e29f) ? _props.waterLine : gameObject->GetPosition().y;
