    ...
    switch (reason) {
        ...
        case LWS_CALLBACK_CLIENT_CONNECTION_ERROR: {
            ...
            if (cit != impl->connections.end()) {
                cit->second.state = (reason == LWS_CALLBACK_CLIENT_CONNECTION_ERROR)
                                     ? WsState::Failed : WsState::Closed;
                if (cit->second.callbacks.onClose)
                    cit->second.callbacks.onClose(0, "");
...
static const struct lws_protocols kProtocols[] = {
    { "ws", lwsCallback, sizeof(WsPerSessionData), 65536, 0, nullptr, 0 },
...
WebSocketClient::WebSocketClient() : _impl(std::make_unique<Impl>()) {
    ...
    if (!_impl->ctx)
        spdlog::error("WebSocketClient: failed to create lws context");
}
...
void WebSocketClient::Send(WsConnectionID id, const std::vector<uint8_t>& data) {
    ...
    if (it->second.wsi && it->second.state == WsState::Open)
...
static EM_BOOL emWsOnError(int, const EmscriptenWebSocketErrorEvent*, void* ud) {
    ...
    ref->impl->socketToId.erase(it != ref->impl->connections.end()
                                  ? it->second.socket : 0);
    delete ref;
    return EM_TRUE;
}
...
static EM_BOOL emWsOnClose(int, const EmscriptenWebSocketCloseEvent* e, void* ud) {
    ...
    if (it != ref->impl->connections.end()) {
        ...
        if (it->second.callbacks.onClose)
            it->second.callbacks.onClose(e->code, e->reason);
...
WebSocketClient::~WebSocketClient() {
    for (auto& [id, conn] : _impl->connections) {
        if (conn.socket)
            emscripten_websocket_close(conn.socket, 1000, "shutdown");
    }
}
...
void WebSocketClient::Send(WsConnectionID id, const std::vector<uint8_t>& data) {
    ...
    emscripten_websocket_send_binary(it->second.socket,
