...
void PhysicsDebugDrawer::drawContactPoint(
    const btVector3& point, const btVector3& normal, btScalar distance, int lifeTime, const btVector3& color
) {
    // TODO: check if singleton exists?
    GraphicsSubsystem::Get()->PushDebugLine(
        { glm::vec3(point.getX(), point.getY(), point.getZ()), glm::vec3(color.getX(), color.getY(), color.getZ()) },
        { glm::vec3(
              point.getX() + distance * normal.getX(),
              point.getY() + distance * normal.getY(),
              point.getZ() + distance * normal.getZ()
          ),
