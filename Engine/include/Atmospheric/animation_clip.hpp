...
struct SkeletonClipHandle {
    uint32_t id = 0;
    bool IsValid() const {
        return id != 0;
    }
    bool operator==(const SkeletonClipHandle& o) const {
        return id == o.id;
    }
};
...
// ── Skeleton clip (skeletal/skinned animation) ───────────────────────────────
// Per-joint local TRS keyframe channels, indexing into a Skeleton by joint
// index. Data-only and shared (AnimationLibrary); SkeletalComponent samples it
// against a Skeleton each frame. Translation/scale interpolate linearly,
// rotation slerps (glTF LINEAR). Empty channels fall back to the joint's bind
// pose, so a clip need only carry the joints it animates.
struct Vec3Key {
    float time = 0.0f;
    glm::vec3 value{ 0.0f };
};
...
// Keyframe samplers (binary search + interpolation; clamp outside the range).
// Empty tracks return the provided fallback (the joint's bind value).
