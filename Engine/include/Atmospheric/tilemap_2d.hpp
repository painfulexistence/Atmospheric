...
public:
    ...
    void Draw(GraphicsSubsystem* gfx, float camX, float camY, int screenW, int screenH) const;

    /// True if the world-space pixel (wx, wy) falls on a solid tile.
    bool IsSolidWorld(float wx, float wy) const;

    int GetPixelWidth() const {
        return _data.width * _data.tileSize;
    }
    int GetPixelHeight() const {
        return _data.height * _data.tileSize;
    }

    const Tilemap2DData& GetData() const {
        return _data;
    }
