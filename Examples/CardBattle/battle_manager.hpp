...
namespace CardGame {
    ...
    public:
        explicit BattleManagerComponent(GameObject*, Application* app) : _app(app), _rng(std::random_device{}()) {
        }
        ...
        CombatantComponent* player() const {
            return _player;
        }
        const std::vector<CombatantComponent*>& enemies() const {
            return _enemies;
        }
        DeckComponent* deck() const {
            return _deck;
        }
        EnergyComponent* energy() const {
            return _energy;
        }
        ...
        int encounterIndex() const {
            return _encounter;
        }
        int encounterCount() const {
            return static_cast<int>(Encounters().size());
        }
        const std::vector<int>& rewardCards() const {
            return _reward;
        }
        float bannerTimer() const {
            return _bannerTimer;
        }
        ...
        void SetTarget(int idx) {
            if (idx >= 0 && idx < static_cast<int>(_enemies.size()) && _enemies[idx]->IsAlive()) _target = idx;
        }
        void CycleTarget() {
        ...
        bool TryPlayCard(int handIdx, int targetIdx) {
            ...
            case TargetKind::AllEnemies:
                for (auto* e : _enemies)
                    if (e->IsAlive()) targets.push_back(e);
        ...
        void OnTick(float dt) override {
            ...
            for (auto* e : _enemies)
                e->hitFlash = std::max(0.0f, e->hitFlash - dt * 3.0f);
    ...
    private:
        ...
        void startEncounter(int idx) {
            ...
            _target = 0;
            pushLog("Encounter " + std::to_string(idx + 1) + " / " + std::to_string(encounterCount()));
        }
        ...
        void applyTargetEffect(const Effect& e, CombatantComponent* caster, CombatantComponent* target) {
            switch (e.kind) {
            ...
            case EffectKind::ApplyVulnerable:
                target->Status()->Add(StatusType::Vulnerable, e.value);
                break;
            case EffectKind::ApplyWeak:
                target->Status()->Add(StatusType::Weak, e.value);
                break;
            case EffectKind::ApplyPoison:
                target->Status()->Add(StatusType::Poison, e.value);
                break;
            default:
                break;
            }
        ...
        void applyCasterEffect(const Effect& e, CombatantComponent* caster) {
            ...
            case EffectKind::Block:
                caster->Health()->AddBlock(e.value);
                break;
            case EffectKind::Heal:
                caster->Health()->Heal(e.value);
                break;
            case EffectKind::GainStrength:
                caster->Status()->Add(StatusType::Strength, e.value);
                break;
        ...
        void generateReward() {
            ...
            for (int i = 0; i < n; ++i)
                _reward.push_back(pool[i]);
        }

        CombatantComponent* pickEnemy(int idx) {
            if (idx >= 0 && idx < static_cast<int>(_enemies.size()) && _enemies[idx]->IsAlive()) return _enemies[idx];
            for (auto* e : _enemies)
