...
inline bool IsTargetEffect(EffectKind k) {
    return k == EffectKind::Damage ||
           k == EffectKind::ApplyVulnerable ||
           k == EffectKind::ApplyWeak ||
...
inline const std::vector<CardDef>& CardDB() {
    static const std::vector<CardDef> db = {
        { 0, "Strike",       1, CardKind::Attack, TargetKind::Enemy,
            {{EffectKind::Damage, 6}},
            "Deal 6 damage." },
        { 1, "Defend",       1, CardKind::Skill,  TargetKind::Self,
            {{EffectKind::Block, 5}},
            "Gain 5 Block." },
        { 2, "Bash",         2, CardKind::Attack, TargetKind::Enemy,
            {{EffectKind::Damage, 8}, {EffectKind::ApplyVulnerable, 2}},
            "Deal 8. Apply 2 Vulnerable." },
        { 3, "Cleave",       1, CardKind::Attack, TargetKind::AllEnemies,
            {{EffectKind::Damage, 8}},
            "Deal 8 to ALL enemies." },
        { 4, "Pommel Strike",1, CardKind::Attack, TargetKind::Enemy,
            {{EffectKind::Damage, 9}, {EffectKind::DrawCards, 1}},
            "Deal 9. Draw 1 card." },
        { 5, "Iron Wave",    1, CardKind::Attack, TargetKind::Enemy,
            {{EffectKind::Damage, 5}, {EffectKind::Block, 5}},
            "Deal 5. Gain 5 Block." },
        { 6, "Inflame",      1, CardKind::Power,  TargetKind::Self,
            {{EffectKind::GainStrength, 2}},
            "Gain 2 Strength." },
        { 7, "Poison Stab",  1, CardKind::Attack, TargetKind::Enemy,
            {{EffectKind::Damage, 3}, {EffectKind::ApplyPoison, 3}},
            "Deal 3. Apply 3 Poison." },
        { 8, "Shrug It Off", 1, CardKind::Skill,  TargetKind::Self,
            {{EffectKind::Block, 8}, {EffectKind::DrawCards, 1}},
            "Gain 8 Block. Draw 1 card." },
        { 9, "Second Wind",  1, CardKind::Skill,  TargetKind::Self,
            {{EffectKind::Heal, 6}, {EffectKind::Block, 4}},
            "Heal 6 HP. Gain 4 Block." },
        {10, "Whirlwind",    2, CardKind::Attack, TargetKind::AllEnemies,
            {{EffectKind::Damage, 5}, {EffectKind::Block, 6}},
            "Deal 5 to ALL. Gain 6 Block." },
        {11, "Heavy Blow",   2, CardKind::Attack, TargetKind::Enemy,
...
inline const CardDef& GetCard(int id) { return CardDB()[id]; }
...
inline const std::vector<EnemyDef>& EnemyDB() {
    static const std::vector<EnemyDef> db = {
        ...
        { "Jaw Worm", 44, {
            ...
            { "Bellow", IntentKind::Buff,   {{EffectKind::GainStrength, 3},
                                             {EffectKind::Block, 6}} },
            { "Thrash", IntentKind::Attack, {{EffectKind::Damage, 7},
                                             {EffectKind::Block, 5}} },
        }},
        ...
        { "Acid Slime", 32, {
            { "Corrosive Spit", IntentKind::Attack, {{EffectKind::Damage, 7},
                                                     {EffectKind::ApplyWeak, 1}} },
