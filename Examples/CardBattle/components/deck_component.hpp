...
namespace CardGame {
    ...
    public:
        ...
        const std::vector<int>& Master() const {
            return _master;
        }
        int DrawCount() const {
            return static_cast<int>(_draw.size());
        }
        int DiscardCount() const {
            return static_cast<int>(_discard.size());
        }

        // Permanently add a card to the deck (post-battle reward).
        void AddCard(int cardId) {
            _master.push_back(cardId);
        }
        ...
        void RemoveFromHand(int handIdx, bool exhaust) {
            ...
            if (exhaust)
                _exhaust.push_back(id);
            else
                _discard.push_back(id);
        }
