...
namespace CardGame {
    ...
    public:
        EnemyBrainComponent(GameObject*, const EnemyDef* def, unsigned seed) : _def(def), _rng(seed) {
            ChooseNextMove();
        }

        std::string GetName() const override {
            return "EnemyBrainComponent";
        }
        ...
        const EnemyMove& TelegraphedMove() const {
            return _def->moves[_moveIdx];
        }

        void ChooseNextMove() {
            const int n = static_cast<int>(_def->moves.size());
            if (n <= 1) {
                _moveIdx = 0;
                return;
            }
