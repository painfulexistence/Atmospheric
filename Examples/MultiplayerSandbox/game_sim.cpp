...
const SpellDef kSpells[static_cast<int>(SpellType::Count)] = {
    { "Spark Bolt", "1", 10 }, { "Fireball", "2", 50 },  { "Grenade", "3", 100 },  { "Water Jet", "4", 4 },
    { "Acid Flask", "5", 90 }, { "Dig Blast", "6", 12 }, { "Rapid Wand", "7", 5 },
};
...
void GameSim::Init(uint32_t seed) {
    ...
    for (int i = 0; i < 2; i++) {
        players[i] = Player{};
        players[i].x = static_cast<float>(world.spawnX[i]);
        players[i].y = static_cast<float>(world.spawnY[i]);
...
bool GameSim::PlayerBoxBlocked(int idx, float px, float py) const {
    int x0 = static_cast<int>(std::floor(px)) - Player::HW;
    int x1 = static_cast<int>(std::floor(px)) + Player::HW;
    int y0 = static_cast<int>(std::floor(py)) - Player::HH;
    int y1 = static_cast<int>(std::floor(py)) + Player::HH;
    for (int y = y0; y <= y1; y++) {
        for (int x = x0; x <= x1; x++) {
...
void GameSim::StepPlayer(int idx, const InputFrame& in) {
    ...
    if (!p.alive) {
        if (tick >= p.respawnAt) {
            p.alive = true;
            p.hp = Player::MAX_HP;
            p.x = static_cast<float>(world.spawnX[idx]);
            p.y = static_cast<float>(world.spawnY[idx]);
            p.vx = p.vy = 0;
    ...
    while (std::abs(remX) > 1e-4f) {
        ...
        if (!PlayerBoxBlocked(idx, p.x + step, p.y)) {
            ...
            for (int up = 1; up <= 3; up++) {
                if (!PlayerBoxBlocked(idx, p.x + step, p.y - static_cast<float>(up))) {
                    p.x += step;
                    p.y -= static_cast<float>(up);
                    climbed = true;
                    break;
                }
            }
    ...
    p.x = std::clamp(p.x, 3.0f, static_cast<float>(SandWorld::W - 4));
    p.y = std::clamp(p.y, 3.0f, static_cast<float>(SandWorld::H - 4));
...
void GameSim::ApplyEnvironmentDamage(int idx) {
    ...
    int x0 = static_cast<int>(p.x) - Player::HW, x1 = static_cast<int>(p.x) + Player::HW;
    int y0 = static_cast<int>(p.y) - Player::HH, y1 = static_cast<int>(p.y) + Player::HH;
    for (int y = y0; y <= y1; y++) {
        for (int x = x0; x <= x1; x++) {
...
void GameSim::DamagePlayer(int idx, int dmg, int from, float kx, float ky) {
    ...
    if (p.hp <= 0) {
        ...
        world.CarveCircle(static_cast<int>(p.x), static_cast<int>(p.y), 4);
        world.PaintCircle(static_cast<int>(p.x), static_cast<int>(p.y), 4, Mat::Smoke, 120);
    }
}
...
void GameSim::SpawnProjectile(SpellType type, int owner, float x, float y, float vx, float vy, int life) {
    for (Projectile& p : projectiles) {
        if (!p.alive) {
            p = Projectile{ true, type, static_cast<uint8_t>(owner), x, y, vx, vy, life };
            return;
        }
    }
    projectiles.push_back(Projectile{ true, type, static_cast<uint8_t>(owner), x, y, vx, vy, life });
}

void GameSim::FireSpell(int idx, const InputFrame& in) {
    Player& p = players[idx];
    auto spell = SpellType(in.spell % static_cast<uint8_t>(SpellType::Count));
    float a = in.Aim();
    float dx = std::cos(a), dy = std::sin(a);
    float mx = p.x + dx * 6.0f;
    float my = p.y + dy * 6.0f;
    p.cooldownUntil = tick + static_cast<uint32_t>(kSpells[static_cast<int>(spell)].cooldownTicks);
    ...
    case SpellType::WaterJet:
        for (int i = 0; i < 2; i++) {
            SpawnProjectile(spell, idx, mx, my, dx * 3.0f + RandSym(0.25f), dy * 3.0f + RandSym(0.25f), 60);
        }
        break;
    ...
    for (Projectile& p : projectiles) {
        ...
        if (--p.life <= 0) {
            if (p.type == SpellType::Grenade) {
                ...
                world.PaintCircle(static_cast<int>(p.x), static_cast<int>(p.y), 2, Mat::Water, 230);
        ...
        int substeps = std::max(1, static_cast<int>(std::ceil(speed)));
        float sx = p.vx / static_cast<float>(substeps);
        float sy = p.vy / static_cast<float>(substeps);
        ...
        for (int s = 0; s < substeps && p.alive; s++) {
            ...
            for (int i = 0; i < 2; i++) {
                Player& pl = players[i];
                if (!pl.alive || i == static_cast<int>(p.owner)) continue;
                if (std::abs(nx - pl.x) <= Player::HW + 1.5f && std::abs(ny - pl.y) <= Player::HH + 1.5f) {
                    ...
                    case SpellType::RapidWand:
                        ...
                        break;
                    case SpellType::Fireball:
                        Explode(nx, ny, 11, 38, p.owner, true);
                        break;
                    case SpellType::AcidFlask:
                        DamagePlayer(i, 8, p.owner, p.vx * 0.1f, p.vy * 0.1f);
                        world.PaintCircle(static_cast<int>(nx), static_cast<int>(ny), 5, Mat::Acid, 200);
                        break;
                    case SpellType::WaterJet:
                        world.PaintCircle(static_cast<int>(nx), static_cast<int>(ny), 2, Mat::Water, 230);
            ...
            // hit terrain
            if (world.SolidAt(static_cast<int>(nx), static_cast<int>(ny))) {
                if (p.type == SpellType::Grenade) {
                    // bounce: pick the axis that hit
                    if (world.SolidAt(static_cast<int>(nx), static_cast<int>(p.y))) {
                        p.vx = -p.vx * 0.5f;
                        sx = -sx * 0.5f;
                    } else {
                        p.vy = -p.vy * 0.45f;
            ...
            // liquids drag / douse
            Mat at = world.GetMat(static_cast<int>(nx), static_cast<int>(ny));
            if (SandWorld::IsLiquid(at)) {
                p.vx *= 0.92f;
                p.vy *= 0.92f;
                if (p.type == SpellType::Fireball && at == Mat::Water) {
                    world.PaintCircle(static_cast<int>(nx), static_cast<int>(ny), 2, Mat::Steam, 180);
                    p.alive = false;
                    break;
                }
            }
...
void GameSim::ImpactProjectile(Projectile& p) {
    ...
    switch (p.type) {
    case SpellType::SparkBolt:
        world.CarveCircle(x, y, 2);
        break;
    case SpellType::RapidWand:
        world.CarveCircle(x, y, 1);
        break;
    case SpellType::Fireball:
        Explode(p.x, p.y, 11, 38, p.owner, true);
        break;
    case SpellType::WaterJet:
        world.PaintCircle(x, y, 2, Mat::Water, 230);
        break;
    case SpellType::AcidFlask:
        world.PaintCircle(x, y, 5, Mat::Acid, 220);
        break;
    default:
        break;
    }
...
void GameSim::Explode(float cx, float cy, int radius, int damage, int owner, bool ignite) {
    world.CarveCircle(static_cast<int>(cx), static_cast<int>(cy), radius);
    if (ignite) {
        world.IgniteCircle(static_cast<int>(cx), static_cast<int>(cy), radius + 2, 160);
    }
    world.PaintCircle(static_cast<int>(cx), static_cast<int>(cy), radius / 2, Mat::Smoke, 60);

    float r = static_cast<float>(radius) * 2.0f;
    for (int i = 0; i < 2; i++) {
        Player& pl = players[i];
        ...
        if (d >= r) continue;
        float t = 1.0f - d / r;
        int dmg = static_cast<int>(static_cast<float>(damage) * t);
...
uint32_t GameSim::Checksum() const {
    ...
    auto mix = [&h](uint32_t v) { h ^= v + 0x9e3779b9u + (h << 6) + (h >> 2); };
    for (int i = 0; i < 2; i++) {
        const Player& p = players[i];
        mix(static_cast<uint32_t>(static_cast<int32_t>(p.x * 16.0f)));
        mix(static_cast<uint32_t>(static_cast<int32_t>(p.y * 16.0f)));
        mix(static_cast<uint32_t>(p.hp));
        mix(static_cast<uint32_t>(p.kills * 7 + p.deaths));
        mix(p.alive ? 1u : 0u);
    }
    mix(tick);
    return h;
