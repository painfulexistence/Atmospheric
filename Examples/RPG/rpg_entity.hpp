...
struct Stats {
    ...
    void takeDamage(int dmg) {
        hp = std::max(0, hp - std::max(1, dmg));
    }
    void heal(int amt) {
        hp = std::min(maxHp, hp + amt);
    }
    void restoreMp(int amt) {
        mp = std::min(maxMp, mp + amt);
    }
};
...
struct Enemy : Entity {
    Enemy() = default;
    Enemy(float startX, float startY, EnemyDef def) : def(std::move(def)) {
        x = startX;
        y = startY;
        w = h = 24;
    }
...
struct BattleState {
    ...
    void tickLog(float dt) {
        for (auto& e : log)
            e.ttl -= dt;
        log.erase(
            std::remove_if(log.begin(), log.end(), [](const BattleLogEntry& e) { return e.ttl <= 0; }), log.end()
        );
    }
...
inline std::vector<uint8_t>
    MakeColorSheetPixels(int tileSize, int cols, int rows, const std::vector<std::array<uint8_t, 3>>& colors) {
    ...
    for (int r = 0; r < rows; r++) {
        for (int c = 0; c < cols; c++) {
            int idx = r * cols + c;
            auto [cr, cg, cb] =
                (idx < static_cast<int>(colors.size())) ? colors[idx] : std::array<uint8_t, 3>{ 128, 128, 128 };
            for (int py = 0; py < tileSize; py++) {
                for (int px_ = 0; px_ < tileSize; px_++) {
                    ...
                    px[i + 1] =
                        static_cast<uint8_t>(std::clamp(static_cast<int>(cg) + (edge ? -30 : noise(rng)), 0, 255));
                    px[i + 2] =
                        static_cast<uint8_t>(std::clamp(static_cast<int>(cb) + (edge ? -30 : noise(rng)), 0, 255));
                    px[i + 3] = 255;
                }
            }
        }
